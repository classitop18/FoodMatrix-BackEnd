...
export const verifyResetToken = async (
    ...
    try {
        ...
        console.log({ token }, "verifying with secret", CONFIG.PASSWORD_RESET_SECRET)
        ...
        // Decode JWT
        const decoded: any = await verifyJwtToken(token, CONFIG.PASSWORD_RESET_SECRET);
        ...
        // Expired session?
        if (!session.isValid || (session.expiresAt && new Date(session.expiresAt) < new Date())) {
...
export const authenticateForChangePassword = async (
    ...
    try {
        ...
        // Decode JWT
        const decoded: any = await verifyJwtToken(reset_password_token, CONFIG.PASSWORD_RESET_SECRET);
