...
import { AIGeneratedRecipe, AIProvider, AIRecipeRequest, NutritionInfo, RECIPE_GENERATION_SYSTEM_PROMPT, RecipeParser, RecipePromptBuilder } from "../interfaces/ai.interfaces";
...
export class AIRecipeService {
    constructor(
        ...
        private recipeStorage: any
    ) { }


    async generatePersonalizedRecipes(request: AIRecipeRequest): Promise<AIGeneratedRecipe[]> {
        try {
            // Validate request
            ...
            const enhancedRequest: AIRecipeRequest = {
                ...
                avoidRecentRecipes: recipeHistory
                    .filter(r => r?.lastInteraction > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)) // Last 7 days
                    .map(r => r?.name)
            };
            ...
            const recipes = this.recipeParser.parse(response.content, enhancedRequest);

            // Post-processing validations
            const validatedRecipes = this.postProcessRecipes(recipes, enhancedRequest);

            console.log({ validatedRecipes, enhancedRequest });

            // Store generated recipes for future learning
            const recipesWithIds: any = await this.recipeStorage.storeAIGeneratedRecipes(validatedRecipes, enhancedRequest);
            ...
            if (message.includes('timeout') || message.includes('ETIMEDOUT')) {
                throw new Error('Recipe generation timed out. Please try again with fewer recipes or simpler requirements.');
            }

            if (message.includes('budget') || message.includes('pantry')) {
                throw new Error(`Constraint error: ${message}. Please adjust your requirements.`);
            }
    ...
    private postProcessRecipes(recipes: AIGeneratedRecipe[], request: AIRecipeRequest): AIGeneratedRecipe[] {
        return recipes.map(recipe => {
            ...
            if (request.maxBudgetPerServing &&
                recipe.costAnalysis.costPerServing > request.maxBudgetPerServing * 1.1) {
                console.warn(`Recipe "${recipe.name}" exceeds budget: ${recipe.costAnalysis.costPerServing}`);
            }
            ...
            if (request.pantryOnly) {
                const nonPantryItems = recipe.ingredients.filter(ing => !ing.isPantryItem);
                if (nonPantryItems.length > 0) {
                    console.warn(`Recipe "${recipe.name}" has non-pantry items in pantry-only mode`);
                }
            }
    ...
    private adjustServings(recipe: AIGeneratedRecipe, targetServings: number): AIGeneratedRecipe {
        ...
        return {
            ...
            costAnalysis: {
                ...
                pantryItemsSavings: (recipe.costAnalysis.pantryItemsSavings || 0) * ratio
            }
        };
    }
    ...
    private estimateNutrition(recipe: AIGeneratedRecipe): NutritionInfo {
        // Fallback nutrition estimation based on meal type
        const baseNutrition: Record<string, NutritionInfo> = {
            breakfast: { calories: 400, protein_g: 15, carbs_g: 50, fat_g: 15, fiber_g: 6, sugar_g: 10 },
            lunch: { calories: 500, protein_g: 25, carbs_g: 55, fat_g: 18, fiber_g: 8, sugar_g: 8 },
            dinner: { calories: 600, protein_g: 30, carbs_g: 60, fat_g: 20, fiber_g: 10, sugar_g: 6 },
            snack: { calories: 200, protein_g: 8, carbs_g: 25, fat_g: 8, fiber_g: 4, sugar_g: 12 }
        };
    ...
    private calculateAvgCost(recipes: AIGeneratedRecipe[]): number {
        const sum = recipes.reduce((acc, r) => acc + r.costAnalysis.costPerServing, 0);
    ...
    private calculatePantryUsage(recipes: AIGeneratedRecipe[]): string {
        ...
        const pantryIngredients = recipes.reduce((acc, r) =>
            acc + r.ingredients.filter(i => i.isPantryItem).length, 0
        );
    ...
    private async convertDatabaseRecipeToAIFormat(
        ...
        const recipeWithIngredients = await this.recipeStorage.getRecipeWithIngredients(dbRecipe.id);

        // Parse instructions (stored as JSON string in DB)
        let instructions: string[] = [];
        try {
            instructions = typeof dbRecipe.instructions === 'string'
        ...
        const ingredients = (recipeWithIngredients?.ingredients || []).map(ing => ({
        ...
        const nutrition: NutritionInfo = dbRecipe.calories
            ? {
                ...
                fat_g: Math.round((dbRecipe.calories * 0.30) / 9),
                fiber_g: 8,
                sugar_g: 6,
                sodium_mg: 600,
                cholesterol_mg: 50
        ...
        const aiRecipe: AIGeneratedRecipe = {
            ...
            ingredients: ingredients.map(ing => ({
                ...ing,
                quantity: servingRatio !== 1 ? this.scaleQuantity(ing.quantity, servingRatio) : ing.quantity,
            ...
            variations: [],
            webSourceInspirations: [`Traditional ${dbRecipe.name} recipe from database`],
     ...
     */
    private categorizeIngredient(dbCategory: string): 'produce' | 'pantry' | 'dairy' | 'protein' | 'seafood' | 'meat' | 'bakery' | 'spices' | 'beverages' | 'frozen' | 'other' {
    ...
    async searchSpecificRecipe(
        ...
        try {
            ...
            const dbRecipes = await this.recipeStorage.searchRecipesByName(recipeName, accountId);

            if (dbRecipes && dbRecipes.length > 0) {
                // Filter by meal type if specified
                const matchingRecipe = dbRecipes.find(r =>
            ...
            const prompt = await this.promptBuilder.buildRecipeSearchPrompt(recipeName, mealType, servings, dietaryRestrictions);
            ...
            const recipe = this.parseRecipeResponse(response.content, recipeName, mealType, servings);
            ...
            if (recipe) {
                ...
                if (accountId) {
                    try {
                        ...
                        const [storedRecipe] = await this.recipeStorage.storeAIGeneratedRecipes(
                            [recipe],
                            storageRequest
                        );
                        ...
                        console.error('⚠️ Failed to store custom recipe in database:', storageError);
    ...
    async searchRecipeVariations(
        ...
        try {
            ...
            const prompt = this.buildVariationsPrompt(recipeName, mealType, variationCount, servings);
            ...
            const recipes = this.parseMultipleRecipes(response.content, recipeName, mealType, servings);
...
Follow all requirements from single recipe search for EACH variation.`;
    ...
    private parseRecipeResponse(
        ...
        try {
            ...
            return this.validateAndEnhanceRecipe(recipe, recipeName, mealType, servings);
    ...
    private parseMultipleRecipes(
        ...
        try {
            ...
            if (!jsonMatch) {
                ...
                const single = this.parseRecipeResponse(content, recipeName, mealType, servings);
    ...
    private validateAndEnhanceRecipe(
        ...
        if (Math.abs(calcCalories - recipe.nutrition.calories) > 50) {
            console.warn('Nutrition math incorrect, recalculating calories');
        ...
        return {
            ...
            instructions: recipe.instructions || ['Recipe instructions not available'],
            costAnalysis: recipe.costAnalysis,
            nutrition: recipe.nutrition,
            nutritionalHighlights: recipe.nutritionalHighlights || ['Nutritious and balanced'],
            ...
            webSourceInspirations: recipe.webSourceInspirations || [`Traditional ${recipeName} recipe`],
            healthConsiderations: recipe.healthConsiderations || [],
            aiReasoningNotes: recipe.aiReasoningNotes || 'Authentic recipe based on traditional preparation methods'
        };
    }
