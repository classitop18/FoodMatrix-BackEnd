...
import { AIRecipeRequest, RecipePromptBuilder, RecipeScore } from '../interfaces/ai.interfaces';



export class AdvancedRecipePromptBuilder implements RecipePromptBuilder {

    private pantryStorage: any;

    constructor(pantryStorage: any) { this.pantryStorage = pantryStorage; }
 ...
 */
    private buildLearningSection(request: AIRecipeRequest): string {
        ...
        const frequentCuisines = this.analyzeCuisinePreferences(request.recipeHistory);
        const preferredIngredients = this.analyzeIngredientPatterns(request.recipeHistory);
 ...
 */
    ...
    private buildOutputFormatSection(request: AIRecipeRequest): string {
        ...
        [
        {
            ...
            ${request.pantryOnly ? `"insufficientPantryReason": "optional: explanation if canGenerateRecipe is false",
    ...
    private buildHealthInfo(request: AIRecipeRequest): string[] {
        ...
        if (request.healthConditions?.length) {
            healthInfo.push(`🩺 Health Conditions: ${request.healthConditions.join(', ')} - Adapt recipe accordingly`);
        }
        if (request.dietaryRestrictions?.length) {
            healthInfo.push(`🥗 Dietary Restrictions: ${request.dietaryRestrictions.join(', ')} - MUST comply`);
        }
        if (request.allergies?.length) {
            healthInfo.push(`⚠️ ALLERGIES (ZERO TOLERANCE): ${request.allergies.join(', ')} - Completely avoid`);
        }
        if (request.healthGoals?.length) {
            healthInfo.push(`🎯 Health Goals: ${request.healthGoals.join(', ')} - Optimize for these`);
        }
    ...
    private buildFoodPreferences(request: AIRecipeRequest): string[] {
        ...
        if (request.excludedFoods?.length) {
            prefs.push(`❌ EXCLUDE (Must Avoid): ${request.excludedFoods.join(', ')}`);
        }
        if (request.customExclusions?.length) {
            prefs.push(`❌ Custom Exclusions: ${request.customExclusions.join(', ')}`);
        }
        if (request.includedFoods?.length) {
            prefs.push(`✅ INCLUDE (Preferred): ${request.includedFoods.join(', ')}`);
        }
        if (request.customInclusions?.length) {
            prefs.push(`✅ Custom Inclusions: ${request.customInclusions.join(', ')}`);
        }

        return prefs;
    }

    private analyzeCuisinePreferences(history: RecipeScore[]): Array<{ cuisine: string, percentage: number, avgScore: number }> {
        const cuisineStats = new Map<string, { count: number, totalScore: number }>();

        history.forEach(recipe => {
            const stats = cuisineStats.get(recipe.cuisineType) || { count: 0, totalScore: 0 };
            stats.count++;
    ...
    async buildRecipeSearchPrompt(
        ...
        const dietarySection = dietaryRestrictions?.length
            ? dietaryRestrictions.map((r: any, index) => {
                return `
Member ${index + 1}:
