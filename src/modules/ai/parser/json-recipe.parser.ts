import { AIGeneratedRecipe, AIRecipeRequest, RecipeParser } from "../interfaces/ai.interfaces";
...
export class JSONRecipeParser implements RecipeParser {
    ...
    private validateAndEnhanceRecipes(recipes: any[], request: AIRecipeRequest): AIGeneratedRecipe[] {
        ...
        return recipes.map((recipe: any, index: number) => {
            ...
            const pantryItemsUsedCount = recipe.pantryItemsUsedCount ??
                (recipe.ingredients?.filter((ing: any) => ing.isPantryItem).length || 0);
            ...
            return {
                ...
                instructions: recipe.instructions || ['Cooking instructions not available'],
                costAnalysis: {
                    totalCost: recipe.costAnalysis?.totalCost || (defaultBudget * targetServings),
                    costPerServing: recipe.costAnalysis?.costPerServing || defaultBudget,
                    budgetEfficiency: Math.max(0, Math.min(1, recipe.costAnalysis?.budgetEfficiency || 0.8)),
                ...
                webSourceInspirations: recipe.webSourceInspirations || ['Popular online recipes'],
                healthConsiderations: recipe.healthConsiderations || [],
                aiReasoningNotes: recipe.aiReasoningNotes || 'Recipe selected based on your preferences',
