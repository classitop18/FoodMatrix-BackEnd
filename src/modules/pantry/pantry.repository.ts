...
import { ingredients, pantryItems, pantryAlerts } from "../../database/schemas/schema.js";
...
interface PantryItemsStorageInterface {
    getPantryItems(accountId: string, params: any): Promise<{ data: (PantryItem & { ingredient: any })[]; total: number }>;
    addPantryItem(pantryItem: InsertPantryItem): Promise<PantryItem>;
    updatePantryItem(pantryItemId: string, updates: Partial<PantryItem>): Promise<PantryItem>;
...
export class PantryItemsStorage implements PantryItemsStorageInterface {
    ...
    async addPantryItem(pantryItem: InsertPantryItem): Promise<PantryItem> {
        const [newPantryItem] = await this.db.insert(pantryItems).values(pantryItem).returning();
        return newPantryItem;
    }

    async updatePantryItem(pantryItemId: string, updates: Partial<PantryItem>): Promise<PantryItem> {
