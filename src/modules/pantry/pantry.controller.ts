...
export class PantryItemsController {
    ...
    private getAccountId(req: AuthenticatedRequest): string {
        ...
        const accountId = (req.headers["x-account-id"] as string) || (req as any).session?.accountId;
        if (!accountId) {
    ...
    getPantryItems = async (
        ...
        try {
            ...
            const result = await this.service.getPantryItems(accountId, validatedQuery);
    ...
    getExpiringItems = async (
        ...
        try {
            ...
            return sendSuccess(res, result, "Expiring items fetched successfully", 200);
        } catch (error) {
            next(error);
        }
    };
    ...
    getPantryAlerts = async (
        ...
        try {
            ...
            return sendSuccess(res, result, "Pantry alerts fetched successfully", 200);
        } catch (error) {
            next(error);
        }
    };
    ...
    addPantryItem = async (
        ...
        try {
            ...
            const addedBy = (req as any).session?.memberId || (req.user as any)?.memberId || null;
