...
export const createPantryItemSchema = z.object({
    ...
    location: z.enum(["refrigerator", "freezer", "pantry", "cabinet", "countertop"], {
...
}).refine((data) => data.ingredientId || (data.ingredientName && data.category), {
    message: "Either select an existing ingredient or provide name and category to create one",
...
export const getPantryItemsQuerySchema = z.object({
    ...
    location: z
        .enum(["refrigerator", "freezer", "pantry", "cabinet", "countertop"])
        .optional(),
    sortBy: z.enum(["createdAt", "expirationDate", "name"]).optional().default("createdAt"),
