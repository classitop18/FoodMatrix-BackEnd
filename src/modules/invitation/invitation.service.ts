...
export class InvitationService {
    ...
    async sendInvitation(data: CreateInvitationDTO, invitedBy: string) {
        ...
        if (existingUser) {
            const isMember = await this.memberRepository.isUserMember(existingUser.id, data.accountId);
            if (isMember) {
                throw Object.assign(new Error("User is already a member of this account"), { statusCode: 400 });
            }
        }

        // Check if an active invitation already exists for this email and account
        const existingInvitation = await this.repository.findPendingByEmailAndAccount(
            data.email,
            data.accountId
        );

        if (existingInvitation) {
            throw Object.assign(new Error("An active invitation already exists for this email"), { statusCode: 409 });
        }
    ...
    async acceptInvitation(data: AcceptInvitationDTO, userEmail: string) {
        ...
        if (invitation.status !== "pending") {
            throw Object.assign(new Error(`Invitation has already been ${invitation.status}`), { statusCode: 400 });
        }
    ...
    async approveInvitation(data: ApproveInvitationDTO, adminId: string) {
        ...
        if (invitation.status !== "user_accepted") {
            throw Object.assign(new Error("Invitation must be accepted by user before approval"), { statusCode: 400 });
        }

        // Find the user by email
        const user = await this.userRepository.findByEmail(invitation.email);
        if (!user) {
            throw Object.assign(new Error("User account not found. User must register before approval."), { statusCode: 404 });
        }
    ...
    async rejectInvitation(data: RejectInvitationDTO, adminId: string) {
        ...
        if (!["pending", "user_accepted"].includes(invitation.status)) {
            throw Object.assign(new Error("Cannot reject this invitation"), { statusCode: 400 });
        }
    ...
    async resendInvitation(data: ResendInvitationDTO, userId: string) {
        ...
        if (!["pending", "expired"].includes(invitation.status)) {
            throw Object.assign(new Error("Cannot resend this invitation"), { statusCode: 400 });
        }
    ...
    async cancelInvitation(data: CancelInvitationDTO, userId: string) {
        ...
        if (invitation.status !== "pending") {
            throw Object.assign(new Error("Can only cancel pending invitations"), { statusCode: 400 });
        }
