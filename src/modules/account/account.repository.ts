...
import { accounts, members } from "../../database/schema.ts";
...
export class AccountRepository implements IAccountRepository {
    ...
    async getAccountById(id: string, primaryAdminId: string): Promise<Account | null> {
        const result = await this.db
            .select()
            .from(accounts)
            .where(
                and(
                    eq(accounts.id, id),
                    eq(accounts.primaryAdminId, primaryAdminId)
                )
            )
            .limit(1);
